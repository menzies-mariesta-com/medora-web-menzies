...
import { staff, notification, hospital, patientVisit, referHistory, status, user } from "./schema";
...
export const patientVisitRelations = relations(patientVisit, ({many}) => ({
	notifications: many(notification),
}));

export const referHistoryRelations = relations(referHistory, ({many}) => ({
	notifications: many(notification),
}));
