...
import * as mf from '$lib/server/heka/administration/manufacturer.server';
...
export async function GET(event: RequestEvent) {
	...
	if (mode === 'search') {
		...
		return json(
			await mf.searchManufacturersForHospital(hospitalId, {
				query: q,
	...
	if (idStr != null && idStr !== '') {
		...
		return json(await mf.getManufacturerById(hospitalId, { id }));
	}
	...
	return json(
		await mf.getManufacturerPaginated(hospitalId, {
			page,
			pageSize,
			search:
...
export async function POST(event: RequestEvent) {
	...
	const statusId =
		rawSt != null && rawSt !== '' && Number.isFinite(Number(rawSt))
			...
			: undefined;
	return json(
		await mf.createManufacturer(hospitalId, {
			name,
...
export async function PUT(event: RequestEvent) {
	...
	return json(
		await mf.updateManufacturer(hospitalId, {
			id,
...
export async function DELETE(event: RequestEvent) {
	...
	await mf.deleteManufacturer(hospitalId, { id });
	return json({ ok: true });
}
