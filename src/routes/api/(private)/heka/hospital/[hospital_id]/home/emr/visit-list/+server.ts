...
export const GET: RequestHandler = async (event) => {
	...
	switch (mode) {
		...
		case 'visit.list': {
			...
			const hospitalName =
				event.url.searchParams.get('hospitalName') ?? undefined;
			const branchName =
				event.url.searchParams.get('branchName') ?? undefined;
			const doctorName =
				event.url.searchParams.get('doctorName') ?? undefined;
			const visitTypeRaw = event.url.searchParams.get('visitType');
			const visitTypeId =
				visitTypeRaw != null && visitTypeRaw.trim() !== ''
					? Number(visitTypeRaw)
					: undefined;
			const visitStatusRaw =
				event.url.searchParams.get('visitStatus');
			const visitStatus = (
				visitStatusRaw != null && visitStatusRaw.trim() !== ''
					? visitStatusRaw
					: undefined
