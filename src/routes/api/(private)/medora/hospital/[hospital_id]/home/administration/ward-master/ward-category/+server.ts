...
import * as wardCategory from '$lib/server/medora/ipd/ward-category.server';
...
export async function GET(event: RequestEvent) {
	...
	if (idStr) {
		...
		return json(await wardCategory.getWardCategoryById({ id }));
	}

	if (event.url.searchParams.get('active') === '1') {
		return json(
			await wardCategory.listActiveWardCategories({ hospitalId })
		);
	}
	...
	return json(
		await wardCategory.getWardCategoryPaginated({
			hospitalId,
			page,
			pageSize,
			name,
...
export async function POST(event: RequestEvent) {
	...
	return json(
		await wardCategory.createWardCategory({
			hospitalId,
			name,
			code: body.code ? String(body.code).trim() : null,
			wardMarkup: String(body.wardMarkup ?? '0'),
			statusId:
...
export async function PUT(event: RequestEvent) {
	...
	return json(
		await wardCategory.updateWardCategory({
			id,
			hospitalId,
			...
			code:
				body.code !== undefined
					...
					: undefined,
			wardMarkup:
				body.wardMarkup !== undefined
					? String(body.wardMarkup)
					: undefined,
			statusId:
...
export async function DELETE(event: RequestEvent) {
	...
	if (!Number.isFinite(id) || id <= 0)
		throw error(400, 'id is required');
	await wardCategory.deleteWardCategory({ id, hospitalId });
	return json({ ok: true });
}
