...
export const GET: RequestHandler = async (event) => {
	...
	const bills = await ensureDb().query.ipBillingTable.findMany({
		where: and(
			eq(table.ipBillingTable.visitId, visitId),
			eq(table.ipBillingTable.hospitalId, hospitalId),
...
export const POST: RequestHandler = async (event) => {
	...
	const bill = await ensureDb().query.ipBillingTable.findFirst({
		where: and(
			eq(table.ipBillingTable.id, billingId),
			eq(table.ipBillingTable.hospitalId, hospitalId),
			ne(table.ipBillingTable.statusId, StatusEnum.DELETED)
		),
		with: {
