...
export async function GET(event: RequestEvent) {
	...
	return json(
		await procedure.listOperativeNotes({
			hospitalId,
...
export async function POST(event: RequestEvent) {
	...
	if (body.action === 'delete') {
		await procedure.deleteOperativeNote({
			id: id(body.id, 'id'),
			hospitalId
		});
		return json({ ok: true });
	...
	return json(
		await procedure.saveOperativeNote({
			id: body.id ? id(body.id, 'id') : undefined,
			hospitalId,
			visitId: id(body.visitId, 'visitId'),
			preOp: String(body.preOp ?? ''),
			findings: String(body.findings ?? ''),
			technique: String(body.technique ?? ''),
			bloodLoss: body.bloodLoss ? String(body.bloodLoss) : null,
			specimens: String(body.specimens ?? ''),
			postOp: String(body.postOp ?? ''),
			surgeonId: staff.id
		})
	);
}
