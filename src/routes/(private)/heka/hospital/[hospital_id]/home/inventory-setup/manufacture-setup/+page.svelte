<script lang="ts">
	...
	import ManufacturerFormModal from '$lib/component/own/local/private/heka/inventory-setup/manufacture-setup/ManufacturerFormModal.svelte';
	...
	let rows = $state<ManufacturerListRow[]>([]);
	let total = $state(0);
	...
	const columns: MariTableColumn<ManufacturerListRow>[] = [
		{
			id: 'id',
			header: m.id(),
	...
	async function openCreate() {
		ManufacturerModalState.mode = 'create';
		ManufacturerModalState.editRow = null;
		const result = await dialogService.open({
			title: m.new_manufacturer(),
			component: ManufacturerFormModal
		});
		if (result.confirmed) fetchRows();
	}

	async function openEdit(row: ManufacturerListRow) {
		ManufacturerModalState.mode = 'edit';
		ManufacturerModalState.editRow = row;
		const result = await dialogService.open({
			title: m.edit_manufacturer(),
			component: ManufacturerFormModal
		});
		if (result.confirmed) fetchRows();
	}

	async function handleDelete(row: ManufacturerListRow) {
		const result = await dialogService.open({
			title: m.delete_manufacturer(),
			message: `Delete "${row.name ?? m.manufacture_setup_title()}"?`,
...
<div class="space-y-6">
	<div class="flex flex-wrap items-center justify-between gap-4">
		<h1 class="text-2xl font-bold">{m.manufacture_setup_title()}</h1>
		...
		<DaisyUiCardBody>
			<div class={TableEnum.HEIGHT}>
				<MariTable
					...
					emptyMessage={m.no_manufacturers_create()}
					showRowActions={true}
