<script lang="ts">
	...
	import PharmacyGenericFormModal from '$lib/component/own/local/private/heka/inventory-setup/pharmacy-generic/PharmacyGenericFormModal.svelte';
	import { PharmacyGenericModalState } from '$lib/state/pharmacy-generic-modal.state.svelte';
	import type {
		PharmacyGenericListRow,
		StatusListRow
	...
	let rows = $state<PharmacyGenericListRow[]>([]);
	let total = $state(0);
	...
	const columns: MariTableColumn<PharmacyGenericListRow>[] = [
		...
		{
			id: 'code',
			header: m.pharmacy_generic_code(),
	...
	async function openCreate() {
		PharmacyGenericModalState.mode = 'create';
		PharmacyGenericModalState.editRow = null;
		const result = await dialogService.open({
			title: m.new_pharmacy_generic(),
			component: PharmacyGenericFormModal
		});
		if (result.confirmed) fetchRows();
	}

	async function openEdit(row: PharmacyGenericListRow) {
		PharmacyGenericModalState.mode = 'edit';
		PharmacyGenericModalState.editRow = row;
		const result = await dialogService.open({
			title: m.edit_pharmacy_generic(),
			component: PharmacyGenericFormModal
		});
		if (result.confirmed) fetchRows();
	}

	async function handleDelete(row: PharmacyGenericListRow) {
		const result = await dialogService.open({
			title: m.delete_pharmacy_generic(),
			message: `Delete "${row.name ?? m.pharmacy_generic_master()}"?`,
		...
		<DaisyUiCardBody>
			<div class={TableEnum.HEIGHT}>
				<MariTable
					...
					emptyMessage={m.no_pharmacy_generics_create()}
					showRowActions={true}
