<script lang="ts">
	...
	async function fetchServiceItems() {
		...
		serviceItems = await fetchJson<ServiceItemListRow[]>(
			`/api/medora/hospital/${hospitalId}/home/administration/service-order/service-item-master?mode=all`
		);
	}
	...
	async function fetchTaggings(forceRefresh = false) {
		...
		try {
			...
			>(
				`/api/medora/hospital/${hospitalId}/home/administration/service-order/service-item-tagging?${qs.toString()}`
			);
			...
			const results = await Promise.all(
				selectedAllowedBranchIds.map((branchId) =>
					fetchJson<ServiceTaggingListRow[]>(
						`/api/medora/hospital/${hospitalId}/home/administration/service-order/service-item-tagging?mode=all&branchId=${encodeURIComponent(
							branchId
			...
			if (mode === 'create') {
				await Promise.all(
					targetBranchIds.map((branchId) =>
						fetchJson(
							`/api/medora/hospital/${hospitalId}/home/administration/service-order/service-item-tagging`,
							{
								method: 'POST',
								body: JSON.stringify({
									branchId,
					...
					<div class={TableEnum.HEIGHT}>
						...
						{:else}
							<div class="overflow-x-auto">
								<table
									class="table-pin-rows table-pin-cols table table-zebra table-sm"
								>
									<thead>
										<tr>
											<th
