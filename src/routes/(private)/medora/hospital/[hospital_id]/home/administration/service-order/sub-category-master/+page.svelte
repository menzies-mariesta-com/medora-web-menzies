<script lang="ts">
	...
	import SubCategoryFormModal from '$lib/component/own/local/private/medora/administration/category/SubCategoryFormModal.svelte';
	...
	async function fetchSubCategories(forceRefresh = false) {
		...
		try {
			...
			>(
				`/api/medora/hospital/${hospitalId}/home/administration/service-order/sub-category-master?${qs.toString()}`
			);
		} finally {
			isLoading = false;
		}
