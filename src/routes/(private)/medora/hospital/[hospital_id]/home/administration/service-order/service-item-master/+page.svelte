<script lang="ts">
	...
	async function fetchCategories() {
		...
		categories = await fetchJson<CategoryListRow[]>(
			`/api/medora/hospital/${hospitalId}/home/administration/service-order/category-master?mode=all`
		);
	}
		...
		const lists = await Promise.all(
			categories.map((cat) =>
				fetchJson<SubCategoryListRow[]>(
					`/api/medora/hospital/${hospitalId}/home/administration/service-order/sub-category-master?mode=all&categoryId=${encodeURIComponent(
						String(cat.id)
					)}`
				)
			)
	...
	async function fetchServiceItems(forceRefresh = false) {
		...
		try {
			...
			>(
				`/api/medora/hospital/${hospitalId}/home/administration/service-order/service-item-master?${qs.toString()}`
			);
		} finally {
			isLoading = false;
		}
	...
	async function handleSubmit(e: Event) {
		...
		try {
			if (mode === 'create') {
				await fetchJson(
					`/api/medora/hospital/${hospitalId}/home/administration/service-order/service-item-master`,
					{
						method: 'POST',
						body: JSON.stringify({
							subCategoryId,
