<script lang="ts">
	...
	const prNewPath = $derived(
		medoraHospitalPageUrl(
			hospitalId,
			'/medora/home/inventory/purchase-requisition/new' as any
		)
	);

	function prViewHref(prId: string) {
		return `/medora/hospital/${hospitalId}/home/inventory/purchase-requisition/${encodeURIComponent(prId)}`;
	}

	function prEditHref(prId: string) {
		return `/medora/hospital/${hospitalId}/home/inventory/purchase-requisition/${encodeURIComponent(prId)}/edit`;
	}

	function prApproveHref(prId: string) {
		const u = medoraHospitalPageUrl(
			hospitalId,
			'/medora/home/inventory/purchase-requisition/approve' as any
		);
	...
	async function submitCancelPrWithReason(reason: string) {
		...
		const res = await fetch(
			`/api/medora/hospital/${hospitalId}/home/inventory/purchase-requisition/cancel`,
			{
				method: 'POST',
