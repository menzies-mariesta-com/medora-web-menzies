<script lang="ts">
	...
	const grnNewPath = $derived(
		medoraHospitalPageUrl(
			hospitalId,
			'/medora/home/inventory/grn/new' as any
		)
	);
	...
	async function loadList() {
		...
		try {
			...
			const res = await fetch(
				`/api/medora/hospital/${hospitalId}/home/inventory/grn?${sp.toString()}`,
	...
	$effect(() => {
		...
		canPostLoading = true;
		fetch(
			`/api/medora/hospital/${hid}/home/inventory/grn?mode=canPost&storeId=${encodeURIComponent(String(sid))}`,
			{ method: 'GET' }
		)
	...
	async function transferToRequestingStore(row: GrnRow) {
		...
		try {
			const res = await fetch(
				`/api/medora/hospital/${hospitalId}/home/inventory/grn/transfer-to-requesting-store`,
				{
					method: 'POST',
