<script lang="ts">
	...
	async function load() {
		...
		try {
			const [res, iumRes] = await Promise.all([
				fetch(
					`/api/medora/hospital/${hospitalId}/home/inventory/purchase-requisition?id=${encodeURIComponent(prId)}`,
					{ method: 'GET' }
				),
				fetch(
	...
	async function closeLine(lineId: number) {
		...
		try {
			const res = await fetch(
				`/api/medora/hospital/${hospitalId}/home/inventory/purchase-requisition/close-line`,
				{
					method: 'POST',
		...
		try {
			const lineAdjustments =
				action === InvApprovalActionEnum.APPROVED
					...
					: undefined;
			const res = await fetch(
				`/api/medora/hospital/${hospitalId}/home/inventory/purchase-requisition/approve`,
				{
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({
	...
	async function resubmit() {
		...
		try {
			const res = await fetch(
				`/api/medora/hospital/${hospitalId}/home/inventory/purchase-requisition/resubmit`,
				{
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
