<script lang="ts">
	...
	import LPatientListViewEditModal from '$lib/component/own/local/private/medora/patient/list/LPatientListViewEditModal.svelte';
	...
	function patientListApiBase(hospitalId: string) {
		return `/api/medora/hospital/${encodeURIComponent(
			hospitalId
	...
	function selectForEmrPatient(patient: PatientWithRelations) {
		if (!hospitalId) return;
		const baseUrl = medoraHospitalPageUrl(
			hospitalId,
			WebRoutesEnum.MEDORA_HOME_NURSING_WORKBENCH_EMR_VITAL
		);
