<script lang="ts">
	...
	let historyBills = $state<IpBillingHistory[]>([]);
	...
	function closeBlockedTooltip(): string {
		if (visitLevelDiscountLocked) {
			return m.ip_billing_bill_already_closed_tooltip();
		}
		const key = billingReadiness?.blockReasonKey ?? null;
		switch (key) {
			case 'nursing_incomplete':
				return m.ip_billing_bill_close_blocked_nursing();
			case 'no_billable_lines':
				return msg.ip_billing_bill_close_blocked_no_lines();
			case 'visit_not_discharged':
				return 'Patient must be discharged before closing the IP bill.';
			default:
				return msg.ip_billing_readiness_not_ready();
		}
	}
	...
	function groupLines(lines: BillingLine[]): BillingGroup[] {
		...
		for (const line of lines) {
			...
			const other = tr(msg.ip_billing_group_other, 'Other');
	...
	async function postIpBillingUpdate(
		payload: Record<string, unknown>
	) {
		const res = await fetch(
			`/api/medora/hospital/${hospitalId}/home/billing/ip-billing/visit-lines`,
			{
				method: 'POST',
	...
	async function loadBillingLines(currentVisitId: string | null) {
		...
		try {
			const res = await fetch(
				`/api/medora/hospital/${hospitalId}/home/billing/ip-billing/visit-lines?visitId=${visitNumeric}`
			);
			if (!res.ok) {
				groups = [];
				grandTotal = 0;
	...
	async function applyDiscount() {
		...
		try {
			const res = await postIpBillingUpdate(payload);
			if (!res.ok) {
				if (res.status === 403) {
					try {
						...
						loadError =
							...
							tr(
								msg.ip_billing_discount_locked_after_print,
					...
					loadError = tr(
						msg.ip_billing_discount_save_failed,
	...
	async function closeOpBill() {
		...
		if (
			...
			return;
		try {
			const res = await postIpBillingUpdate({
				action: 'close',
				visitId: visitNumeric
			});
			if (!res.ok) {
				try {
					...
					loadError =
						...
						(res.status === 400
							? closeBlockedTooltip()
							: tr(msg.ip_billing_load_failed, 'Request failed.'));
				} catch {
					loadError =
						res.status === 400
							? closeBlockedTooltip()
	...
	async function printBill(opts: {
		...
		const bodyHtml = buildOpBillingPrintBodyHtml({
			...
			labels: {
				service: tr(msg.ip_billing_print_service, 'Service'),
				order: tr(msg.ip_billing_print_order, 'Order'),
				amount: tr(msg.ip_billing_print_amount, 'Amount'),
				subtotal: tr(msg.ip_billing_print_subtotal, 'Subtotal'),
				grandTotal: tr(msg.ip_billing_print_grand_total, 'Grand total'),
				discount: tr(msg.ip_billing_print_discount, 'Discount'),
				netTotal: tr(msg.ip_billing_print_net_total, 'Net total')
			},
			formatMoney: formatMoneyAmount
		});

		const printedAt = `${tr(msg.ip_billing_print_generated, 'Printed')}: ${new Date().toLocaleString()}`;

		await printFromDocumentMaster({
			hospitalId,
			documentCode: DOCUMENT_PRINT_CODE.IP_BILL,
			extraPlaceholders: {
				...
				'{{hospital.name}}':
					v?.hospitalName?.trim() ||
					tr(msg.ip_billing_title, 'IP Billing'),
				'{{print.body_html}}': bodyHtml,
				'{{print.datetime}}': printedAt,
				'{{print.label_patient}}': tr(
					msg.ip_billing_print_patient,
					'Patient'
				),
				'{{print.label_patient_code}}': tr(
					msg.ip_billing_print_patient_code,
					'Patient code'
				),
				'{{print.label_visit_no}}': tr(
					msg.ip_billing_print_visit_no,
					'Visit no.'
				),
				'{{print.label_date}}': tr(msg.ip_billing_print_date, 'Date'),
				'{{print.label_doctor}}': tr(
					msg.ip_billing_print_doctor,
					'Doctor'
				),
				'{{print.label_branch}}': tr(
					msg.ip_billing_print_branch,
					'Branch'
				),
				'{{print.label_thank_you}}': tr(
					msg.ip_billing_print_thank_you,
					'Thank you for choosing us.'
				)
			},
			iframeId: 'ip-billing-print-iframe'
		});
	}
	...
	async function loadHistoryBills(
		...
		try {
			const res = await fetch(
				`/api/medora/hospital/${hospitalId}/home/billing/ip-billing/bills?visitId=${visitNumeric}`
			);
			if (!res.ok) {
				historyBills = [];
	...
	async function printHistoryBill(billingId: number): Promise<void> {
		...
		try {
			const res = await fetch(
				`/api/medora/hospital/${hospitalId}/home/billing/ip-billing/bills`,
				{
					method: 'POST',
					headers: { 'content-type': 'application/json' },
...
<WashCard>
	<WashCardBody className="gap-4">
		<div class="flex flex-wrap items-center justify-between gap-3">
			<WashCardBodyTitle
				>{msg.ip_billing_title()}</WashCardBodyTitle
			>
			{#if visitId}
				<div class="flex flex-wrap items-center gap-2">
					<WashTooltip
						tooltipText={canCloseIpBill
							? tr(msg.ip_billing_bill_close, 'Bill close')
							: closeBlockedTooltip()}
						className=""
					>
						<WashButton
					...
					<WashTooltip
						tooltipText={msg.ip_billing_print_bill()}
						className=""
					>
						<WashButton
		...
		{#if !visitId}
			<p class="text-sm text-base-content/70">
				{msg.ip_billing_select_visit_hint()}
			</p>
		{:else}
			...
			<div class="mt-2 space-y-3">
				...
				{:else if groups.length === 0}
					<p class="text-sm text-base-content/60">
						{tr(
							msg.ip_billing_no_services_found,
				...
				{:else}
					...
					>
						<span>{tr(msg.ip_billing_total_bill, 'Total bill')}</span>
					...
					{#if billingMeta?.discountedByStaff || billingMeta?.printedByStaff}
						...
						>
							{#if billingMeta?.discountedByStaff}
								<div>
									{tr(msg.ip_billing_discounted_by, 'Discounted by')}
							...
							{#if billingMeta?.printedByStaff}
								<div>
									{tr(msg.ip_billing_printed_by, 'Printed by')}
											...
											>
												<div class="min-w-0">
													<p class="truncate font-medium">
														{line.serviceName ??
															tr(
																msg.ip_billing_service_fallback,
																'Service'
															)}
													</p>
...
{#if isDiscountModalOpen}
	<WashDialog
		id="ip-billing-discount-modal"
		open={true}
		onClose={() => (isDiscountModalOpen = false)}
		title={tr(msg.ip_billing_discount_title, 'Discount')}
		description={tr(
			msg.ip_billing_discount_subtitle,
	...
	>
			<div class="mt-1 space-y-3">
				<label class="form-control w-full">
					<div class="label">
						<span class="label-text">
							{tr(
								msg.ip_billing_discount_type_label,
								'Discount type'
							)}
						</span>
					</div>
				...
				<label class="form-control w-full">
					<div class="label">
						<span class="label-text">
							{tr(msg.ip_billing_discount_amount_label, 'Amount')}
						</span>
						<span class="label-text-alt text-base-content/60">
							{#if discountType === BillingDiscountTypeEnum.PERCENT}
								{tr(
									msg.ip_billing_discount_amount_hint_percent,
									'0–100'
								)}
							{:else if discountType === BillingDiscountTypeEnum.FIXED_AMOUNT}
								{tr(
									msg.ip_billing_discount_amount_hint_amount,
									'Cannot exceed total'
								)}
							{:else}
								{tr(msg.ip_billing_discount_amount_hint_none, '—')}
							{/if}
						</span>
					</div>
					<input
						...
						step="0.01"
						placeholder={tr(
							msg.ip_billing_discount_amount_placeholder,
							'0'
						)}
				...
				<div class="rounded-box bg-base-200/40 p-3 text-sm">
					<div class="flex items-center justify-between gap-3">
						<span class="text-base-content/70">
							{tr(msg.ip_billing_discount_preview_label, 'Preview')}
						</span>
					...
					>
						<span
							>{tr(
								msg.ip_billing_discount_net_total_label,
								'Net total'
							)}</span
						>
						<span class="font-mono tabular-nums">
			...
			<div class="modal-action">
				...
				>
					{tr(msg.ip_billing_discount_clear, 'Clear')}
				</WashButton>
				<WashButton
				...
				>
					{tr(msg.ip_billing_discount_apply, 'Apply')}
				</WashButton>
			</div>
	</WashDialog>
{/if}
