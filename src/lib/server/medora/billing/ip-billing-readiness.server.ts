...
import { getNursingIncompleteLineCountForVisit } from '$lib/server/medora/emr/nursing-complete.server';
...
export function ipBillingCloseBlockedMessage(
...
export async function getIpBillingReadiness(
	event: RequestEvent,
	input: {
		hospitalId: string;
		visitId: number;
		...
		visitStatusTaggingId?: number | null;
	}
	...
	if (input.billAlreadyClosed) {
		...
		blockReasonKey = 'visit_not_discharged';
