...
import type {
	UserGroupPageSchema,
	UserGroupPageSchemaInsert,
	UserGroupPageSchemaUpdate
...
export const getUserGroupPage = query(
	async (): Promise<UserGroupPageSchema[]> => {
...
// get count
export const getUserGroupPageCount = query(
...
export const getUserGroupPagePaginated = query(
	...
	): Promise<PaginatedResult<UserGroupPageSchema>> => {
...
export const getUserGroupPageById = query(
	...
	async ({
		...
		id: number;
	}): Promise<UserGroupPageSchema | null> => {
		const [row] = await ensureDb()
			.select()
			.from(table.userGroupPageTable)
...
export const createUserGroupPage = command(
	'unchecked' as const,
	async (
		payload: UserGroupPageSchemaInsert
	): Promise<UserGroupPageSchema> => {
		const [row] = await ensureDb()
