...
export const getStaffDepartment = query(async (): Promise<StaffDepartmentSchema[]> => {
	const data = await ensureDb().select().from(table.staffDepartmentTable);
	return data;
});

// get count
export const getStaffDepartmentCount = query(async (): Promise<number> => {
	const [row] = await ensureDb().select({ count: count() }).from(table.staffDepartmentTable);
	return row?.count ?? 0;
});
...
export const getStaffDepartmentPaginated = query(
	'unchecked' as const,
	async (params?: PaginationParams): Promise<PaginatedResult<StaffDepartmentSchema>> => {
		const { page, pageSize, limit, offset } = normalizePagination(params);
		const [data, countResult] = await Promise.all([
			ensureDb().select().from(table.staffDepartmentTable).limit(limit).offset(offset),
			ensureDb().select({ count: count() }).from(table.staffDepartmentTable),
		]);
...
export const deleteStaffDepartment = command(
	...
	async ({ id }: { id: number }): Promise<void> => {
		await ensureDb().delete(table.staffDepartmentTable).where(eq(table.staffDepartmentTable.id, id));
		getStaffDepartment().refresh();
	}
);
