...
// get paginated
export const getHospitalPaginated = query(
	'unchecked' as const,
	async (params?: PaginationParams): Promise<PaginatedResult<HospitalSchema>> => {
		const { page, pageSize, limit, offset } = normalizePagination(params);
		const [data, countResult] = await Promise.all([
			ensureDb().select().from(table.hospitalTable).limit(limit).offset(offset),
			ensureDb().select({ count: count() }).from(table.hospitalTable),
		]);
