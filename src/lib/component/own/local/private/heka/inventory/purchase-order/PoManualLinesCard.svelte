<script lang="ts">
	...
	export let manualLineItemFilter: string;
	...
	<DaisyUiCardBody className="gap-4">
		<div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
			...
			<div class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
				...
				/>
				<DaisyUiButton type="button" className="d-btn-primary" onClick={() => onAddItem()}>
		...
		<div class="flex flex-wrap items-center justify-between gap-3 border-t border-base-200 pt-4">
			<div class="text-sm opacity-80">
				Total items: <span class="font-semibold">{totalCount}</span>
			</div>
		</div>
	</DaisyUiCardBody>
</DaisyUiCard>
