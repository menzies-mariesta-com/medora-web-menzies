<script lang="ts">
	...
	import type { UnitTypeListRow } from '$lib/model/type/heka/ui-rows.type';
...
{:else}
	<form onsubmit={handleSubmit} class="flex flex-col gap-4">
		...
		>
			<DaisyUiLabel forText="um-name" className="shrink-0 sm:w-40"
				>{m.unit_master_name()}
		...
		<div
			class="flex min-w-0 flex-col gap-1 sm:flex-row sm:items-center sm:gap-3"
		>
			<DaisyUiLabel forText="um-type" className="shrink-0 sm:w-40"
				>{m.unit_master_unit_type()}</DaisyUiLabel
			>
			<div class="max-w-lg flex-1">
				<DaisyUiSelect
					id="um-type"
