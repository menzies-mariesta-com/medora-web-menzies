...
<div class="space-y-3">
	...
	<div class="grid grid-cols-1 gap-4 xl:grid-cols-2">
		<div class="flex min-h-0 flex-col gap-2">
			...
			{#if readOnly}
				<pre
					class="d-textarea-bordered d-textarea {minHeightClass} w-full overflow-auto font-mono text-xs whitespace-pre-wrap"
				>{html || 'No content'}</pre>
			{:else}
				<textarea
					class="d-textarea-bordered d-textarea {minHeightClass} w-full font-mono text-sm"
					bind:value={html}
					{placeholder}
				></textarea>
			{/if}
