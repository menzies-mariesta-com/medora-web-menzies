<script lang="ts">
	...
	function goToPrescriptionPage() {
		...
		const path = `/medora/hospital/${hospitalId}/home/consultation/cpoe/prescription?visitId=${visitId}`;
	...
	function dashboardApiUrl(visitIdValue: number): string {
		return `/api/medora/hospital/${hospitalId}/home/nursing-workbench/emr/patient-visit-history-dashboard?visitId=${visitIdValue}`;
	}
	...
	function goToCaseSheet(row: VisitTableRow) {
		...
		const path = `/medora/hospital/${hospitalId}/home/nursing-workbench/emr/case-sheet${query}`;
	...
	{:else}
		...
		>
			...
			<div class="flex min-h-0 min-w-0 flex-col gap-6 lg:col-span-8">
				<!-- Visit History -->
				<section
					class="card min-w-0 overflow-hidden border border-base-300 bg-base-100 shadow-sm"
				>
					<div
				...
				<!-- Clinical Case Sheet -->
				<section
					class="card min-w-0 border border-base-300 bg-base-100 shadow-sm"
				>
								...
								{:else}
									<div class="flex flex-wrap gap-2">
										{#each vitalSymptoms as symptom (symptom)}
											<span
												class="badge gap-1 badge-outline border-error/30 bg-error/5"
											>
												<span
