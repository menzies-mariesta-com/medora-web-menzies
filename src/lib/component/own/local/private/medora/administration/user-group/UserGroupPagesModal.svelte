<script lang="ts">
	...
	let allPages = $state<MedoraPageRow[]>([]);
	let allModules = $state<MedoraPageModuleRow[]>([]);
	...
	const pageById = $derived.by(() => {
		const map = new Map<number, MedoraPageRow>();
...
>
	...
	{:else}
		...
		>
			<WashCardBody className="gap-3 overflow-hidden p-4 min-w-0">
