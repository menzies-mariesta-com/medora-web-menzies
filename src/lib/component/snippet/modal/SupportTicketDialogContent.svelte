<script lang="ts">
	...
	let listResult = $state<PaginatedResult<SupportTicketSchema> | null>(
		null
	);
...
<div class="flex h-full min-h-[60vh] flex-col gap-0">
	...
	<div class="flex flex-wrap gap-1 border-b border-base-200 px-4 py-2">
		<DaisyUiButton
			...
			>
				...
				{:else}
					...
					<dl class="grid gap-1 text-sm">
						...
						<dd>
							{detail.requester?.name ?? '—'} ({detail
								.requester?.email ?? detail.requesterId})
						</dd>
						...
						{#if detail.contextUrl}
							...
							<dd class="break-all text-xs">{detail.contextUrl}</dd>
						{/if}
					</dl>
					<div class="flex flex-col gap-1">
					...
					{#if isAdmin}
						...
						<div class="flex flex-col gap-2">
							...
							<DaisyUiSelect className="w-full" bind:value={adminStatus}>
								<option value={SupportTicketStatusEnum.OPEN}>
									{m.support_status_open()}
								</option>
								<option
									value={SupportTicketStatusEnum.IN_PROGRESS}
								>
									{m.support_status_in_progress()}
								</option>
								<option
									value={SupportTicketStatusEnum.RESOLVED}
								>
									{m.support_status_resolved()}
								</option>
								<option value={SupportTicketStatusEnum.CLOSED}>
