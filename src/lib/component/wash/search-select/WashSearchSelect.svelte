<script lang="ts">
	...
	const listboxId = 'wash-search-select-listbox';
...
<div class="relative w-full" bind:this={containerEl}>
	<input
		id={inputId}
		type="text"
		class="input-bordered input w-full {className ?? ''}"
		bind:this={inputEl}
		value={inputValue}
		{placeholder}
		{disabled}
	...
	{#if open}
		<ul
			id={listboxId}
			class="menu absolute right-0 left-0 z-50 flex max-h-60 w-full flex-col overflow-x-hidden overflow-y-auto rounded-box border bg-base-100 shadow-lg {placement ===
			'up'
				? 'bottom-full mb-1'
				: 'top-full mt-1'}"
			role="listbox"
