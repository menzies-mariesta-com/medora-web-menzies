<script lang="ts">
	...
	const visitId = $derived(ObservationPatientDocumentDialogState.visitId);
	const patientId = $derived(ObservationPatientDocumentDialogState.patientId);
	...
	async function searchDocuments(
		...
		return all
			.filter((d: DocumentWithRelations) => {
				...
				const type =
					(d.documentType?.documentType ?? '').toLowerCase();
				return (
					code.includes(q) ||
					num.includes(q) ||
					type.includes(q) ||
