<script lang="ts">
	...
	const patientId = $derived(ObservationDiagnosisDialogState.patientId);
	const diagnosisId = $derived(ObservationDiagnosisDialogState.diagnosisId);
	...
	async function handleSubmit() {
		...
		try {
			...
			toastService.addToast(m.observation_emr_saved(), StatusColorEnum.SUCCESS);
			...
			{#each types as t (t.id)}
				<label class="d-label cursor-pointer justify-start gap-3">
					<input
						...
						class="d-radio d-radio-sm d-radio-primary shrink-0"
						value={String(t.id)}
