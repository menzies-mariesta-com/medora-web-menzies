<script lang="ts">
	...
	const patientId = $derived(ObservationFormEntryDialogState.patientId);
	...
	async function handleSave() {
		...
		if (!desc) {
			toastService.addToast('Description is required.', StatusColorEnum.ERROR);
			return;
		}
		isSubmitting = true;
		try {
			...
			toastService.addToast(
				(err instanceof Error ? err.message : m.observation_emr_save_failed()) as string,
				StatusColorEnum.ERROR
			);
		} finally {
			isSubmitting = false;
