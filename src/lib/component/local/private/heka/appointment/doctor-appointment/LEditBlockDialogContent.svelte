...
<div class="flex flex-col gap-4">
	<div class="flex min-w-0 flex-col gap-1 sm:flex-row sm:items-center sm:gap-3">
		<DaisyUiLabel forText="edit-block-date" className="shrink-0 sm:w-28">Date</DaisyUiLabel>
		<input id="edit-block-date" type="date" bind:value={date} min={todayString} class="d-input d-input-bordered d-input-sm max-w-80 flex-1" />
	</div>
	<div class="flex min-w-0 flex-col gap-1 sm:flex-row sm:items-center sm:gap-3">
		<DaisyUiLabel forText="edit-block-start" className="shrink-0 sm:w-28">From time</DaisyUiLabel>
		<input id="edit-block-start" type="time" bind:value={startTime} class="d-input d-input-bordered d-input-sm max-w-80 flex-1" />
	</div>
	<div class="flex min-w-0 flex-col gap-1 sm:flex-row sm:items-center sm:gap-3">
		<DaisyUiLabel forText="edit-block-end" className="shrink-0 sm:w-28">To time</DaisyUiLabel>
		<input id="edit-block-end" type="time" bind:value={endTime} class="d-input d-input-bordered d-input-sm max-w-80 flex-1" />
	</div>
	{#if isBlockStartInPast}
		<p class="text-sm text-error">Cannot set block time in the past.</p>
	{/if}
	<div class="d-modal-action flex justify-end gap-2 border-t border-base-300 pt-4">
		...
		<button type="button" class="d-btn d-btn-error" onclick={() => handleConfirm()} disabled={!date.trim() || !startTime || !endTime || toHHmm(endTime) <= toHHmm(startTime) || isBlockStartInPast}>Save</button>
	</div>
</div>
