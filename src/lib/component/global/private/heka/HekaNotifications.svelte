<script lang="ts">
	...
	import DaisyUiIndicator from '$lib/component/library/daisyui/indicator/DaisyUiIndicator.svelte';
	import DaisyUiIndicatorItem from '$lib/component/library/daisyui/indicator/item/DaisyUiIndicatorItem.svelte';
	...
	let { hospitalId: hospitalIdProp = undefined }: { hospitalId?: string | null } =
		$props();
...
<div class="z-50 overflow-visible">
	...
	>
		<DaisyUiIndicator className="relative overflow-visible">
			{#if unreadCount > 0}
				<DaisyUiIndicatorItem className="d-badge d-badge-sm d-badge-error border-0 text-[10px] leading-none">
	...
	<DaisyUiModalBox className="max-w-2xl" showCloseButton={false}>
		...
		>
			<div class="min-w-0 flex-1 pe-2">
				<h3 class="text-lg font-semibold leading-tight">Notifications</h3>
				...
				<div class="flex flex-col gap-2">
					{#each modalItems as item (item.id)}
						...
						>
							...
							{#if item.readAt == null}
								<span class="ms-auto h-2 w-2 rounded-full bg-error"></span>
							{/if}
						</DaisyUiButton>
					{/each}
				</div>
